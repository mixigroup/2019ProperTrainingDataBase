...
Example_share *ha_kiyoshi::get_share() {
  Example_share *tmp_share;

  DBUG_ENTER("ha_kiyoshi::get_share()");
...
static handler *example_create_handler(handlerton *hton, TABLE_SHARE *table,
  ...
  return new (mem_root) ha_kiyoshi(hton, table);
}

ha_kiyoshi::ha_kiyoshi(handlerton *hton, TABLE_SHARE *table_arg)
...
*/
static st_handler_tablename ha_kiyoshi_system_tables[] = {
    {(const char *)NULL, (const char *)NULL}};
...
static bool example_is_supported_system_table(const char *db,
  ...
  systab = ha_kiyoshi_system_tables;
...
int ha_kiyoshi::open(const char *, int, uint, const dd::Table *) {
  DBUG_ENTER("ha_kiyoshi::open");
...
int ha_kiyoshi::close(void) {
  DBUG_ENTER("ha_kiyoshi::close");
  DBUG_RETURN(0);
}
...
int ha_kiyoshi::write_row(uchar *) {
  DBUG_ENTER("ha_kiyoshi::write_row");
  /*
...
*/
int ha_kiyoshi::update_row(const uchar *, uchar *) {
  DBUG_ENTER("ha_kiyoshi::update_row");
...
int ha_kiyoshi::delete_row(const uchar *) {
  DBUG_ENTER("ha_kiyoshi::delete_row");
...
int ha_kiyoshi::index_read_map(uchar *, const uchar *, key_part_map,
                               enum ha_rkey_function) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::index_read");
...
int ha_kiyoshi::index_next(uchar *) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::index_next");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
int ha_kiyoshi::index_prev(uchar *) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::index_prev");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
*/
int ha_kiyoshi::index_first(uchar *) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::index_first");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
/**
  ...
  opt_range.cc, opt_sum.cc, sql_handler.cc and sql_select.cc
*/
int ha_kiyoshi::index_last(uchar *) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::index_last");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
/**
  ...
  sql_update.cc
*/
int ha_kiyoshi::rnd_init(bool) {
  DBUG_ENTER("ha_kiyoshi::rnd_init");
  kiyoshi_flag = 0;
  zundoko_flag = 1;
  stats.records = 0;
  DBUG_RETURN(0);
...
int ha_kiyoshi::rnd_end() {
  DBUG_ENTER("ha_kiyoshi::rnd_end");
  DBUG_RETURN(0);
}
...
int ha_kiyoshi::rnd_next(uchar *buf) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::rnd_next");
  ...
  if (kiyoshi_flag == 0) {
    for (Field **field = table->field; *field; field++) {
      buffer.length(0);
      if (rnd_dist(rnd_engine) > 0.5) {
        for (const char* p = "ズン"; *p; ++p) {
          buffer.append(*p);
        }
        zundoko_flag <<= 1;
      } else {
        for (const char* p = "ドコ"; *p; ++p) {
          buffer.append(*p);
        }
        if (zundoko_flag & 16) {
          kiyoshi_flag = 1;
    ...
    rc = 0;
    stats.records++;
  } else if (kiyoshi_flag == 1) {
    for (Field **field = table->field; *field; field++) {
      buffer.length(0);
      for (const char* p = "キ・ヨ・シ！"; *p; ++p) {
        buffer.append(*p);
      }
      (*field)->store(buffer.ptr(), buffer.length(), buffer.charset(), CHECK_FIELD_IGNORE);
    }
...
*/
void ha_kiyoshi::position(const uchar *) {
  DBUG_ENTER("ha_kiyoshi::position");
  DBUG_VOID_RETURN;
}
...
*/
int ha_kiyoshi::rnd_pos(uchar *, uchar *) {
  int rc;
  DBUG_ENTER("ha_kiyoshi::rnd_pos");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
/**
  ...
  sql_show.cc, sql_table.cc, sql_union.cc and sql_update.cc
*/
int ha_kiyoshi::info(uint) {
  DBUG_ENTER("ha_kiyoshi::info");
  DBUG_RETURN(0);
}
...
/**
  ...
  ha_innodb.cc
*/
int ha_kiyoshi::extra(enum ha_extra_function) {
  DBUG_ENTER("ha_kiyoshi::extra");
  DBUG_RETURN(0);
}
...
*/
int ha_kiyoshi::delete_all_rows() {
  DBUG_ENTER("ha_kiyoshi::delete_all_rows");
...
*/
int ha_kiyoshi::external_lock(THD *, int) {
  DBUG_ENTER("ha_kiyoshi::external_lock");
  DBUG_RETURN(0);
}
...
*/
THR_LOCK_DATA **ha_kiyoshi::store_lock(THD *, THR_LOCK_DATA **to,
...
*/
int ha_kiyoshi::delete_table(const char *, const dd::Table *) {
  DBUG_ENTER("ha_kiyoshi::delete_table");
...
*/
int ha_kiyoshi::rename_table(const char *, const char *, const dd::Table *,
                             dd::Table *) {
  DBUG_ENTER("ha_kiyoshi::rename_table ");
  DBUG_RETURN(HA_ERR_WRONG_COMMAND);
}
...
*/
ha_rows ha_kiyoshi::records_in_range(uint, key_range *, key_range *) {
  DBUG_ENTER("ha_kiyoshi::records_in_range");
...
int ha_kiyoshi::create(const char *name, TABLE *, HA_CREATE_INFO *,
                       dd::Table *) {
  DBUG_ENTER("ha_kiyoshi::create");
  /*
